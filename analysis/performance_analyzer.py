...
from data.schemas import TradePlanSchema, PositionSchema, OrderSchema
...
class PerformanceAnalyzer:
    ...
    def add_trade(self, plan: TradePlanSchema, position: PositionSchema, 
                  orders: List[OrderSchema]) -> None:
    ...
    def _calculate_execution_quality(self, entry_order: OrderSchema,
                                   exit_order: OrderSchema,
                                   plan: TradePlanSchema) -> float:
    ...
    def _calculate_plan_adherence(self, position: PositionSchema,
                                plan: TradePlanSchema,
                                entry_order: OrderSchema,
                                exit_order: OrderSchema) -> float:
    ...
    def _calculate_max_drawdown(self, position: PositionSchema,
                              orders: List[OrderSchema]) -> float:
    ...
    def _calculate_max_profit(self, position: PositionSchema,
                            orders: List[OrderSchema]) -> float:
    ...
    def _update_overall_metrics(self) -> None:
        ...
        for symbol in set(t.symbol for t in self.trades):
            ...
            symbol_metrics[symbol] = {
                ...
                "average_win": sum(t.pnl for t in symbol_trades if t.win) / len([t for t in symbol_trades if t.win]) if any(t.win for t in symbol_trades) else 0,
                "average_loss": sum(t.pnl for t in symbol_trades if not t.win) / len([t for t in symbol_trades if not t.win]) if any(not t.win for t in symbol_trades) else 0,
                "profit_factor": abs(sum(t.pnl for t in symbol_trades if t.win) / sum(t.pnl for t in symbol_trades if not t.win)) if sum(t.pnl for t in symbol_trades if not t.win) != 0 else 0
            }
            
        # Calculate metrics by strategy
        strategy_metrics = {}
        for strategy in set(t.plan.strategy for t in self.trades if hasattr(t, 'plan')):
            strategy_trades = [t for t in self.trades if hasattr(t, 'plan') and t.plan.strategy == strategy]
            strategy_metrics[strategy] = {
                ...
                "average_win": sum(t.pnl for t in strategy_trades if t.win) / len([t for t in strategy_trades if t.win]) if any(t.win for t in strategy_trades) else 0,
                "average_loss": sum(t.pnl for t in strategy_trades if not t.win) / len([t for t in strategy_trades if not t.win]) if any(not t.win for t in strategy_trades) else 0,
                "profit_factor": abs(sum(t.pnl for t in strategy_trades if t.win) / sum(t.pnl for t in strategy_trades if not t.win)) if sum(t.pnl for t in strategy_trades if not t.win) != 0 else 0
            }
