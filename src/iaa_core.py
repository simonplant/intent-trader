...
class IntentProcessor:
    ...
    INTENT_PATTERNS = {
        "PLAN": [
            r"morning call", r"market analysis", r"plan.*day", r"levels.*today",
            r"bias.*market", r"key.*ideas", r"setup.*today", r"what.*levels"
        ],
        "FOCUS": [
            r"focus.*on", r"trading.*plan", r"prioritize", r"best.*setup",
            r"which.*trade", r"entry.*strategy", r"plan.*trade"
        ],
        "EXECUTE": [
            r"buy.*shares", r"sell.*shares", r"enter.*position", r"place.*order",
            r"go.*long", r"go.*short", r"size.*position", r"execute.*trade"
        ],
        "MANAGE": [
            r"adjust.*stop", r"move.*target", r"check.*position", r"current.*pnl",
            r"exit.*position", r"close.*trade", r"how.*doing", r"position.*status"
        ],
        "REVIEW": [
            r"session.*review", r"how.*did", r"performance", r"what.*learned",
            r"analyze.*trades", r"daily.*summary", r"review.*day"
        ],
        "COACH": [
            r"improve.*trading", r"feedback", r"what.*wrong", r"better.*next",
            r"pattern.*recognition", r"behavioral.*insight", r"coaching"
        ]
    }
    ...
    def extract_symbols(self, text: str) -> List[str]:
        ...
        excluded = {'THE', 'AND', 'FOR', 'ARE', 'BUT', 'NOT', 'YOU', 'ALL', 'CAN', 'HER', 'WAS', 'ONE', 'OUR', 'HAD', 'BY', 'UP', 'DO', 'NO', 'IF', 'MY', 'ON', 'IT', 'IS', 'IN', 'TO', 'OF', 'AS', 'AT', 'BE', 'OR', 'AN', 'HE', 'HAS', 'HIS', 'THAT', 'WHICH', 'SHE', 'HIM', 'BEEN', 'THAN', 'ITS', 'GET', 'VERY', 'DIM', 'SET', 'OWN', 'UNDER', 'LAST', 'RIGHT', 'THINK', 'EACH', 'SHOULD', 'SAID', 'OVER', 'BACK', 'OTHER', 'MANY', 'THEN', 'THEM', 'THESE', 'SO', 'SOME', 'HER', 'WOULD', 'MAKE', 'LIKE', 'INTO', 'TIME', 'HAS', 'TWO', 'MORE', 'VERY', 'WHAT', 'KNOW', 'JUST', 'FIRST', 'GET', 'MAY', 'NEW', 'WAY', 'COULD', 'THERE', 'USE', 'YOUR', 'HOW', 'SAID', 'EACH', 'WHICH', 'THEIR', 'WILL', 'ABOUT', 'IF', 'UP', 'OUT', 'MANY', 'THEN', 'THEM', 'THESE', 'SO', 'SOME', 'HER', 'WOULD', 'MAKE', 'LIKE', 'INTO', 'HIM', 'HAS', 'TWO', 'MORE', 'GO', 'NO', 'WAY', 'COULD', 'MY', 'THAN', 'FIRST', 'BEEN', 'CALL', 'WHO', 'ITS', 'NOW', 'FIND', 'LONG', 'DOWN', 'DAY', 'DID', 'GET', 'COME', 'MADE', 'MAY', 'PART'}
...
class IAA:
    ...
    def _handle_intent(self, intent: str, message: str, symbols: List[str], 
                      quantities: List[int], prices: List[float]) -> str:
    ...
    def _handle_plan(self, message: str, symbols: List[str]) -> str:
        ...
        if symbols:
            try:
                ...
                price_lines = [f"• {symbol}: ${price:.2f}" for symbol, price in prices.items() if price > 0]
                if price_lines:
                    price_info = f"\n\n📊 **Current Prices:**\n" + "\n".join(price_lines)
            except Exception as e:
...
*Ready to move to EXECUTE phase when plan is confirmed.*"""
    ...
    def _handle_execute(self, message: str, symbols: List[str], 
                       quantities: List[int], prices: List[float]) -> str:
        ...
        if position_data and position_data['entry_price']:
            ...
            if validation['valid']:
                ...
                if position:
                    execution_summary.append(f"✅ {symbol}: {side.upper()} {quantity} shares @ ${entry_price:.2f}")
                    ...
                    if position_data.get('take_profit'):
                        execution_summary.append(f"   Take Profit: ${position_data['take_profit']:.2f}")
            else:
        ...
        if positions:
            ...
            try:
                ...
                for pos in positions:
                    ...
                    position_lines.append(f"• {pos.symbol}: {pos.side.upper()} {pos.quantity}@${pos.entry_price:.2f} → ${pos.current_price:.2f} {pnl_str}")
            except Exception as e:
                self.logger.warning(f"Could not update prices: {e}")
                for pos in positions:
                    position_lines.append(f"• {pos.symbol}: {pos.side.upper()} {pos.quantity}@${pos.entry_price:.2f}")
