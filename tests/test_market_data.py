...
def market_data_feed():
    ...
    feed = MarketDataFeed(
        get_config_manager(), get_log_manager().get_logger(__name__), str(cache_dir)
    )
...
def test_parameter_validation(market_data_feed):
    ...
    with pytest.raises(ValueError):
        market_data_feed.get_historical_data(
            "AAPL", datetime.now(), datetime.now(), source="invalid"
        )
...
def test_data_caching(market_data_feed):
    ...
    cache_file = (
        market_data_feed.cache_dir / f"AAPL_1d_{start_date.date()}_{end_date.date()}.parquet"
    )
