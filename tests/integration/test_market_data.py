...
def test_data_caching(market_data_feed, tmp_path):
    ...
    data1 = market_data_feed.get_historical_data(
        symbols=[symbol], start_date=start_date, end_date=end_date, interval="1m"
    )
    ...
    cache_file = (
        Path(market_data_feed.cache_dir)
        / f"{symbol}_1m_{start_date.date()}_{end_date.date()}.parquet"
    )
