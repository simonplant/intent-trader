...
class ManageAgent:
    ...
    def _adjust_position(self, request: ManagementRequest, position: Dict[str, Any], 
                        market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Adjust position parameters (stop loss, take profit).
        """
        # Check if we've exceeded maximum adjustments
        adjustment_count = len(self.storage.get_history("adjustments", 
            start_time=datetime.now().replace(hour=0, minute=0, second=0)))
        if adjustment_count >= self.max_adjustments:
            return {
                "status": "error",
                "message": "Maximum daily adjustments exceeded"
            }
        ...
        self.storage.add_to_history("adjustments", {
    ...
    def _scale_position(self, request: ManagementRequest, position: Dict[str, Any],
                       market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        pnl_pct = (current_price - entry_price) / entry_price if position["side"] == "long" else \
        ...
        self.storage.save_position(position["position_id"], position)
        self.storage.add_to_history("scales", {
            "position_id": position["position_id"],
    ...
    def _close_position(self, request: ManagementRequest, position: Dict[str, Any],
                       market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Close the position.
        """
        ...
        current_price = market_data["price"]
        entry_price = position["entry_price"]
        pnl = (current_price - entry_price) * position["quantity"] if position["side"] == "long" else \
              (entry_price - current_price) * position["quantity"]

        # Add to history and remove position
        self.storage.add_to_history("trades", {
            "position_id": position["position_id"],
            "symbol": position["symbol"],
            "side": position["side"],
            "quantity": position["quantity"],
