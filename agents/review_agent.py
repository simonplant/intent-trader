...
class ReviewAgent:
    ...
    def _get_trade_history(self, start_time: datetime, end_time: datetime, 
    ...
    def _analyze_execution(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for trade in trades:
            ...
            execution_quality["order_types"][order_type] = execution_quality["order_types"].get(order_type, 0) + 1
        ...
        if execution_quality["slippage"]:
            execution_quality["average_slippage"] = sum(execution_quality["slippage"]) / len(execution_quality["slippage"])
        if execution_quality["time_to_fill"]:
            execution_quality["average_time_to_fill"] = sum(execution_quality["time_to_fill"]) / len(execution_quality["time_to_fill"])
    ...
    def _analyze_management(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for trade in trades:
            ...
            scales = self.storage.get_history("scales", 
            ...
            adjustments = self.storage.get_history("adjustments",
                start_time=datetime.fromisoformat(trade["order"]["timestamp"]),
                end_time=datetime.fromisoformat(trade.get("exit_time", datetime.now().isoformat())))
        ...
        if management_quality["scaling_effectiveness"]:
            management_quality["average_scaling_effectiveness"] = sum(management_quality["scaling_effectiveness"]) / len(management_quality["scaling_effectiveness"])
        if management_quality["adjustment_effectiveness"]:
            management_quality["average_adjustment_effectiveness"] = sum(management_quality["adjustment_effectiveness"]) / len(management_quality["adjustment_effectiveness"])
